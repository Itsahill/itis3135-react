...
export default function Intros() {
  ...
  function renderValue(val) {
    ...
    const extractImageFromString = (s) => {
      ...
      const isImageUrl =
        /(^data:image\/)|\.(png|jpe?g|gif|webp|avif|svg)(\?|$)/i.test(t);
      if (isImageUrl) return t;
      // markdown ![alt](url)
      const md = /!\[([^\]]+)\]\(([^)]+)\)/.exec(t);
      if (md && md.length >= 3) return md[2];
      // html <img src="...">
      const html = /<img[^>]+src=["']?([^"' >]+)["']?[^>]*>/i.exec(t);
      if (html && html.length >= 2) return html[1];
      // plain URL inside text
      const urlMatch = /(https?:\/\/[^\s"'<>]+)/i.exec(t);
      if (
        urlMatch &&
        /\.(png|jpe?g|gif|webp|avif|svg)(\?|$)/i.test(urlMatch[0])
      )
        return urlMatch[0];
      return null;
    };
    ...
    if (typeof val === "string") {
      ...
      if (imgUrl)
        return (
          <img src={imgUrl} alt="student media" className="student-image" />
        );
    ...
    if (typeof val === "object") {
      ...
      if (img)
        return <img src={img} alt="student media" className="student-image" />;
  ...
  const formatName = (student, idx) => {
    ...
    const nameVal =
      ...
      null;
    if (!nameVal)
      return student.email || student.username || `Student ${idx + 1}`;
  ...
  function goNext() {
    ...
    if (Array.isArray(filteredIndices) && filteredIndices.length > 0) {
      const pos = filteredIndices.indexOf(currentIndex);
      const nextPos =
        pos >= 0 && pos < filteredIndices.length - 1
          ? pos + 1
  ...
  const getFullNameString = (student) => {
    ...
    if (typeof nv === "object") {
      const pref =
        nv.preferred || nv.preferredName || nv.preferred_name || null;
  ...
  const { filteredStudents, filteredIndices } = useMemo(() => {
    if (!Array.isArray(students))
      return { filteredStudents: [], filteredIndices: [] };
        ...
        <div className="card card--full">
          <div className="controls">
            ...
            <div style={{ fontSize: "0.9rem", color: "#333" }}>
              {Array.isArray(filteredIndices) && filteredIndices.length > 0
        ...
        <div className="filters">
          {checkboxKeys.map((k) => (
            <label key={k}>
              <input
                type="checkbox"
                checked={!!showFields[k]}
                onChange={() =>
                  setShowFields((prev) => ({ ...prev, [k]: !prev[k] }))
                }
              />
              <span>{k}</span>
            </label>
        ...
        {Array.isArray(students) && currentIndex != null
          ? // single-student view
            (() => {
              ...
              const name = formatName(student, currentIndex);
              const email =
                student.email || student.username || student.id || null;
              const photo =
                student.photo ||
                student.avatar ||
                student.image ||
